...
from core.constants import LOG_DATEFMT

IS_GCP: bool = any(
    [
        os.getenv(key="K_SERVICE"),  # Cloud Run
        os.getenv(key="FUNCTION_TARGET"),  # Cloud Functions
        os.getenv(key="GAE_SERVICE"),  # App Engine
    ]
)

LOG_LEVEL: str = os.getenv(
    key="LOG_SEVERITY_LEVEL",
    default="DEBUG",
).upper()
...
class ColoredFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        ...
        asctime: str = (
            f"\033[1m{self.formatTime(record=record, datefmt=self.datefmt)}{reset}"
        )
...
def set_logger() -> logging.Logger:
    logger: logging.Logger = logging.getLogger(name="partyup-be")
